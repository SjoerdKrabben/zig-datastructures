...
pub fn AVLTree(comptime T: type) type {
    return struct {
        ...
        const Node = struct { left: ?*Node, right: ?*Node, depth: usize, data: T };
        ...
        // fn balanceTree(self: *Self) void {
        //
        // }

        fn rotateRight(node: *Node) void {
            const prev_root = node;
            node = node.left;
            if (node.left) |left_node| {
                left_node.depth -= 1;
            }
            if (node.right) |right_node| {
                prev_root.left = right_node;
            }
            node.right = prev_root;
        }
